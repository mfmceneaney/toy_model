...
std::vector<double> getFitParams(int fit_idx, int nevents) {
   ...
   RooGenericPdf gen("gen", "1.0+0.747*1.0*x[0]*x[1]*(x[2])", RooArgList(h,x,a0));
   
   // G e n e r a t e   e v e n t s
   // -----------------------------
 
   // Generate a dataset of 1000 events in x from gen
   std::unique_ptr<RooDataSet> data{gen.generate(RooArgSet(h,x), nevents)};
   ...
   RooDataSet data_plus("data_hplus", "data_hplus", &*data, RooArgSet(h,x), "h>0");
   RooDataSet data_minus("data_hminus", "data_hminus", &*data, RooArgSet(h,x), "h<0");

   // U s e   B a s i c   R O O T   F i t t i n g
   // -------------------------------------------

   // Set parametres
   std::string fitformula = "1.0/(0.747*1.0)*x*([0])";
   int nbinsx = 8;
   int nbinsy = 8;
   double xmin = -1.0;
   double xmax = 1.0;
   ...
   int nparams = 1;
   double params[1] = {0.1}; // initial parameters of fit function before fitting
   std::string fitopt = "S";

   // Create asymmetry histograms
   TH1D* h_plus  = new TH1D("h_plus","h_plus",nbinsx,xmin,xmax);
   TH1D* h_minus = new TH1D("h_minus","h_minus",nbinsx,xmin,xmax);
   data_plus.fillHistogram(h_plus,RooArgList(x));
   data_minus.fillHistogram(h_minus,RooArgList(x));
   ...
   TCanvas *c = new TCanvas(cname.c_str(), cname.c_str(), 1600, 800);
   c->cd();
...
void ToyModelRepeatAsymmetryPdf1D() {
